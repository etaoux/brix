KISSY.add('brix/gallery/calendar/index', function(S, Brick, Overlay, Page, Brix_Date) {
    var tmpl = '<div class="calendar-pages ks-overlay-content"><div class="calendar-container-pages"></div>	{{#isQuick}}	<div class="calendar-quick"> {{{quick_html}}}	</div>	{{/isQuick}}</div>{{#isOperator}}<div bx-tmpl="calendar" bx-datakey="notLimited,multiSelect,showTime,op_html" class="calendar-operator"> {{{op_html}}}</div>{{/isOperator}}';
...
KISSY.add('brix/gallery/calendar/time', function(S, Brick) {
    ...
    var tmpl = '<div tabIndex="1"><div class="calendar-time"> 时间：<span bx-tmpl="h" bx-datakey="h" class="h">{{h}}</span>:<span bx-tmpl="m" bx-datakey="m" class="m">{{m}}</span>:<span bx-tmpl="s" bx-datakey="s" class="s">{{s}}</span><div class="calendar-time-updown"><i class="iconfont u">&#456</i><i class="iconfont d">&#459</i></div></div><div class="calendar-time-popup"><div bx-tmpl="time" bx-datakey="list" class="calendar-time-popup-bd"> {{#list}} <a class="item">{{.}}</a> {{/list}} </div><i class="iconfont icon-close">&#223</i></div></div>';
