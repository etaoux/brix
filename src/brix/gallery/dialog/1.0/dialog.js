KISSY.add("brix/gallery/dialog/1.0/dialog", function(S,Pagelet,Overlay) {
    function _getContent(v,dir){
        var wrapper = {
                    'up': [
                        '<div class="popup popup-up">',
                            '<div class="popup-left"></div>',
                            '<div class="popup-center">',
                                '<div class="popup-content">'+v+'</div>',
                            '</div>',
                            '<div class="popup-right"></div>',
                            '<div class="popup-bottom">',
                                '<div class="popup-bottom-left"></div>',
                                '<div class="popup-bottom-center">&nbsp;</div>',
                                '<div class="popup-bottom-right"></div>',
                            '</div>',
                        '</div>'
                    ],
                    'right': [
                        '<div class="popup-h popup-h-right">',
                            '<div class="popup-h-top"></div>',
                            '<div class="popup-h-center">',
                                '<div class="popup-h-body">',
                                    '<div class="popup-content">'+v+'</div>',
                                '</div>',
                            '</div>',
                            '<div class="popup-h-bottom"></div>',
                        '</div>'
                    ]
                };
    ...
    Dialog.ATTRS = {
        ...
        dir:{
            getter:function(){
                ...
                dir = start.left === end.left &&
                            ( start.top < end.top ? 'down' : 'up' )
                      || start.top === end.top &&
                            ( start.left < end.left ? 'right' : 'left' )
                        || 'left';
                return dir;
            }
        },
